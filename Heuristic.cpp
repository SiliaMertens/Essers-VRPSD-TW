	...
	for (int vehicle_id = 0; vehicle_id < p.n_vehicles; vehicle_id++) {
		for (int position = 1; position < s.routes[vehicle_id].route.size(); position++) {
			if (customer_id == s.routes[vehicle_id].route[position]) {
				...
				int route_position = s.routes[vehicle_id].route[position];
...
void write_output_file_perturbation(struct problem& p, struct solution& s) {

	ofstream output_file_2;

	output_file_2.open(("Perturbation " + data_file + " day " + p.collection_date + ".txt"), std::ios_base::app);

	output_file_2 << "Data file: " << data_file << endl << " Day " << p.collection_date << endl << " Total Cost: " << s.total_cost << "\n";

	output_file_2 << endl << endl;
	output_file_2.close();
